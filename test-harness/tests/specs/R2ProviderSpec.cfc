		...
		describe( "R2 S3 HTTP contract", () => {
			...
			it( "signs actual HTTP requests, preserves binary bytes, and issues private and public URLs", () => {
				var process = createObject( "java", "java.lang.ProcessBuilder" )
					.init(
						javacast(
							...
							[
								"python3",
								expandPath( "/tests/resources/r2_contract_server.py" )
							]
						)
					)
				...
				try {
					...
					while ( !reader.ready() && process.isAlive() && getTickCount() < deadline ) {
						sleep( 25 );
					}
					expect( reader.ready() ).toBeTrue();
					var port = val( reader.readLine() );
					...
					expect( publicDisk.url( "logos/logo.png" ) ).toBe(
						"http://assets.example.test/logos/logo.png"
					);
